import { dbTribes } from "@/firebaseTribes";
import { addDoc, collection, getDocs, query, where } from "firebase/firestore";
import { logToSheet } from "@/lib/sheetLog";
import React, { useEffect, useRef, useState } from "react";

const EmailModal = ({ open, setOpen, downloadType }) => {
  ...
  const handleSubmit = async () => {
    ...
    const isSchedule = downloadType === "program-schedule";
    const fileUrl = isSchedule
      ? "/assets/program_schedule.pdf"
      : // Winter brochure, served from this repo for now.
        // TEMPORARY: the intended home is Google Drive, so the brochure can be
        // swapped without a code change. Once someone has edit access to
        // https://drive.google.com/file/d/1jN-CUjugj9DPVHE7PTXF844y7n220Oun/view
        // upload the new edition there via Manage versions (keeps the same id)
        // and point this back at the Drive uc?export=download URL.
        "/assets/winter-brochure-tfg.pdf";
    const fileName = isSchedule
      ? "program_schedule.pdf"
      : "TribesforGOOD-Winter-Brochure.pdf";

    // Record the lead BEFORE handing over the file. On iOS Safari starting a
    // download can unload the page, which was cutting off everything that ran
    // after it: mobile downloads were reaching neither Firebase nor the Sheet.
    logToSheet({
      type: "brochure",
      email,
      phone,
      school: schoolName,
      source: downloadType || "brochure",
    });

    const recordLead = (async () => {
      try {
        const emailCol = collection(dbTribes, "emails");
        const existing = await getDocs(
          query(emailCol, where("email", "==", email)),
        );
        // Every download is recorded, including repeat ones. Previously a known
        // email saved nothing at all, so re-engagement left no trace.
        await addDoc(emailCol, {
          email,
          phone,
          schoolName,
          downloadType: downloadType || "brochure",
          isReturning: !existing.empty,
          timestamp: new Date(),
        });
      } catch (error) {
    ...
    // Give the database a moment, but never hold the download hostage: if
    // Firestore is slow or blocked, the visitor still gets their file.
    await Promise.race([
      recordLead,
      new Promise((resolve) => setTimeout(resolve, 2500)),
    ]);

    // Same-origin file, so a plain download link is enough. Fetching it into a
    // blob first was needless for a 4MB PDF and behaves poorly on iOS.
